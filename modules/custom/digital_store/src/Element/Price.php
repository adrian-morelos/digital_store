...
namespace Drupal\digital_store\Element;
...
class Price extends FormElement {
  ...
  public function getInfo() {
    ...
    return [
      ...
      '#allow_negative' => FALSE,
      '#attached' => [
        'library' => ['digital_store/price_admin'],
      ],
      '#element_validate' => [
  ...
  /**
   * Builds the price form element.
   *
   * @param array $element
   *   The initial price form element.
   ...
   *
   * @return array
   *   The built price form element.
   *
   * @throws \InvalidArgumentException
   *   Thrown when #default_value is not an instance of
   *   \Drupal\digital_store\Price.
   */
  public static function processElement(array $element, FormStateInterface $form_state, array &$complete_form) {
    $default_value = $element['#default_value'];
    if (isset($default_value) && !self::validateDefaultValue($default_value)) {
      throw new \InvalidArgumentException('The #default_value for a price element must be an array with "number" and "currency_code" keys.');
    }

    /** @var \Drupal\Core\Config\Entity\ConfigEntityStorageInterface $currency_storage */
    $currency_storage = \Drupal::service('entity_type.manager')->getStorage('currency');
    /** @var \Drupal\digital_store\Entity\CurrencyInterface[] $currencies */
