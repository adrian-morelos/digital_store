...
use Drupal\digital_store\Exception\CurrencyMismatchException;

/**
 * Provides a value object for monetary values.
 *
 * Use the digital_store.currency_formatter service to format prices.
 */
final class Price {
  ...
  public function __toString() {
    $number = Calculator::trim($this->number);
    $default = $number . ' ' . $this->currencyCode;
    $code = $this->currencyCode;
    $currency = \Drupal::entityTypeManager()->getStorage('currency')->load($code);
    if (empty($currency)) {
      return $default;
    }
    ...
    $price = number_format($number, $fraction_digits, '.', ',');
    return '<div class="price"><sup>' . $symbol . '</sup>' . $price . '</div>';
  }
   ...
   *
   * @param \Drupal\digital_store\Price $first_price
   *   The first price.
   * @param \Drupal\digital_store\Price $second_price
   *   The second price.
   *
   * @throws \Drupal\digital_store\Exception\CurrencyMismatchException
